...
def delete_account(nama):
    """
    Hapus user beserta akun & palm_templates-nya (ON DELETE CASCADE).
    CATATAN: kalau user ini punya riwayat di `transactions`, penghapusan
    akan GAGAL karena FK constraint (transactions.account_id tidak pakai
    CASCADE) -- disengaja, supaya audit trail finansial tidak hilang.
    Return True kalau ditemukan & dihapus, False kalau nama tidak ada.
    """
    with db_transaction() as cur:
        cur.execute("DELETE FROM users WHERE nama = %s RETURNING user_id", (nama,))
        row = cur.fetchone()
